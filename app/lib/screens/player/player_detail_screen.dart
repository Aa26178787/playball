...
class _PlayerDetailScreenState extends State<PlayerDetailScreen> {
            ...
            children: [
              Row(children: [
                ...
                Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  box(120, 20), const SizedBox(height: Space.sm), box(80, 14),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          _favLoading
              ? Padding(padding: const EdgeInsets.all(Space.md), child: SizedBox(width: 20, height: 20, child: CircularProgressIndicator(strokeWidth: 2, color: SemColor.brand(context))))
              : IconButton(
              ...
              child: Column(
                ...
                children: [
                  Text('상대전적 — ${player['name']} vs ${isBatter ? '투수' : '타자'}',
                      style: const TextStyle(fontSize: Typo.subtitle, fontWeight: Typo.bold)),
                  const SizedBox(height: Space.md),
                  TextField(
          ...
          if (games == 0)
            ...
            Card(
              child: Padding(
                ...
                child: Column(
                  children: [
                    Text('$games경기 · $pa타석 직접 맞대결', style: const TextStyle(fontSize: Typo.small, color: Colors.grey)),
                    const SizedBox(height: Space.md),
                    Row(
    ...
    return Row(mainAxisSize: MainAxisSize.min, children: [
      ...
      Flexible(
        child: Text(
          ...
          style: TextStyle(fontSize: Typo.caption, color: color, fontWeight: Typo.bold),
        ),
      ),
    ]);
  }
        ...
        child: Stack(children: [
          ...
          Positioned(
            ...
            child: Opacity(
              opacity: 0.07,
              child: Text('$number',
                  style: const TextStyle(fontSize: 150, fontWeight: Typo.black,
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      Text(name,
                          maxLines: 1, overflow: TextOverflow.ellipsis,
                          style: const TextStyle(fontSize: Typo.h1, fontWeight: Typo.extra,
                      ...
                      Row(children: [
                        Flexible(
                          child: Text('$team · $posOrType · #$number',
                              maxLines: 1, overflow: TextOverflow.ellipsis,
                              style: TextStyle(fontSize: Typo.small, color: Colors.white.withValues(alpha: 0.82))),
                        ),
                        // 등록말소/부상자명단 배지 — 백넘버 옆
                            ...
                            GestureDetector(
                              ...
                              child: Container(
                                ...
                                child: const Center(child: Text('!',
                                    style: TextStyle(fontSize: Typo.mini, fontWeight: Typo.black,
                                        color: Colors.white, height: 1))),
                              ),
                            ),
                        ]),
  ...
  static const Map<String, (String, String)> _comparePhrase = {
    ...
    'woba': ('타격을 종합한 득점생산력이 리그 평균보다 {p}% 낮아요', '타격을 종합한 득점생산력이 리그 평균보다 {p}% 높아요'),
    'wrc_plus': ('리그 평균보다 득점생산력이 {p}% 낮아요', '리그 평균보다 득점생산력이 {p}% 높아요'),
    'iso': ('리그 평균보다 순수장타력이 {p}% 낮아요', '리그 평균보다 순수장타력이 {p}% 높아요'),
    'risp': ('리그 평균보다 득점권타율이 {p}% 낮아요', '리그 평균보다 득점권타율이 {p}% 높아요'),
    'gpa': ('리그 평균보다 타격생산력이 {p}% 낮아요', '리그 평균보다 타격생산력이 {p}% 높아요'),
    'bb_k': ('볼넷 대비 삼진(선구안)이 리그 평균보다 {p}% 낮아요', '볼넷 대비 삼진(선구안)이 리그 평균보다 {p}% 높아요'),
    'fpct': ('리그 평균보다 수비성공률이 {p}% 낮아요', '리그 평균보다 수비성공률이 {p}% 높아요'),
    'bb_pct': ('리그 평균보다 볼넷비율이 {p}% 낮아요', '리그 평균보다 볼넷비율이 {p}% 높아요'),
    'babip': ('인플레이 타구가 안타가 된 비율이 리그 평균보다 {p}% 낮아요', '인플레이 타구가 안타가 된 비율이 리그 평균보다 {p}% 높아요'),
  };

  // 리그 평균 대비 친화 문장 + 우열(better).
  // rate 스탯 = 평균 대비 상대 %(평균이 안정적). counting 스탯 = 평균(거의0) 대비 %가 폭발 → '리그 상위 X%' 백분위로.
  (String, bool) _compareText(String key, Map? c, dynamic pv, bool isPitcher) {
    ...
    if (isRate) {
      ...
      final tpl = key == 'k_pct'
          ? ('리그 평균보다 삼진비율이 {p}% 낮아요', '리그 평균보다 삼진비율이 {p}% 높아요')
          : _comparePhrase[key];
      if (tpl == null) {
  ...
  static const Map<String, String> _statExplainMap = {
    ...
    'fip': '수비영향을 뺀 투수 본인실력의 실점력. 낮을수록 좋아요.',
    'avg_against': '상대 타자에게 허용한 타율. 낮을수록 잘 막아요.',
    'bb_per_9': '9이닝당 내준 볼넷 수. 낮을수록 제구가 좋아요.',
    'k_per_9': '9이닝당 잡아낸 삼진 수. 높을수록 탈삼진 능력이 좋아요.',
    'k_bb_pct': '삼진비율에서 볼넷비율을 뺀 값. 높을수록 좋아요.',
    'wins': '투수가 거둔 승리 수.',
    'losses': '투수가 기록한 패배 수. 적을수록 좋아요.',
    'saves': '리드를 끝까지 지켜 경기를 마무리한 횟수(보통 3점 이하 리드). 마무리투수 지표예요.',
    'holds': '세이브 상황에서 리드를 지킨 뒤 다음 투수에게 넘긴 횟수. 중간계투 지표예요.',
    ...
    'woba': '타격결과를 득점가치로 환산한 종합지표. 높을수록 좋아요.',
    'wrc_plus': '리그 평균을 100으로 둔 득점생산력. 100보다 높으면 평균 이상이에요.',
    'iso': '순수장타력(장타율 − 타율). 높을수록 장타가 많아요.',
    'risp': '득점권(주자 2·3루) 상황에서의 타율.',
    'gpa': '출루·장타를 합쳐 본 타격생산력. 높을수록 좋아요.',
        ...
        return Padding(
          ...
          child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
            Row(children: [
              ...
              Text(label, style: TextStyle(fontSize: Typo.title, fontWeight: Typo.extra, color: ink)),
            ]),
            const SizedBox(height: Space.md),
            Text(txt, style: TextStyle(fontSize: Typo.subtitle, height: 1.55, color: sub)),
          ]),
        );
      },
    );
      ...
      builder: (_) => AlertDialog(
        ...
        actions: [
          ...
          TextButton(onPressed: () => Navigator.pop(context, true),
              child: const Text('신고', style: TextStyle(fontWeight: Typo.bold))),
        ],
      ),
    );
    if (ok != true) return;
    ...
    showModalBottomSheet(
      ...
      builder: (ctx) {
        Widget tile(IconData icon, String title, String desc, VoidCallback onTap) => ListTile(
          ...
          title: Text(title, style: TextStyle(fontSize: Typo.subtitle, fontWeight: Typo.bold, color: ink)),
          subtitle: Text(desc, style: TextStyle(fontSize: Typo.small, color: sub)),
  ...
  void _showCompareBubble(Offset pos, String key, Map? c, dynamic pv, bool isPitcher, Color tc) {
    ...
    const tailH = 9.0;
    const gap = 6.0;
    final sentStyle = TextStyle(fontSize: 14.5, fontWeight: Typo.extra, height: 1.4, color: mainCol);
    final explainStyle = TextStyle(fontSize: Typo.small, height: 1.45, color: sub);
    final avgStyle = TextStyle(fontSize: Typo.small, color: sub);
  ...
  Widget _seasonGridSection(String title, List<(String, String, String)> items, {bool isPitcher = false}) {
    ...
    Widget statCard((String, String, String) s) {
      return Container(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(children: [
            Flexible(child: Text(s.$1, style: TextStyle(fontSize: Typo.mini, fontWeight: Typo.medium, color: labelCol))),
          ...
          const SizedBox(height: 6),
          Text(s.$2, style: TextStyle(fontSize: Typo.h2, fontWeight: Typo.extra, color: ink,
    ...
    return Padding(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text(title, style: TextStyle(fontSize: Typo.mini, fontWeight: Typo.bold, color: sub, letterSpacing: 0.5)),
        const SizedBox(height: Space.md),
        GridView.count(
    ...
    showModalBottomSheet(
      ...
      builder: (ctx) => StatefulBuilder(builder: (ctx, setSheet) {
        ...
        return Padding(
          ...
          child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text('핵심 스탯 4개 선택', style: TextStyle(fontSize: Typo.title, fontWeight: Typo.extra, color: ink)),
            const SizedBox(height: Space.xs),
            Text('${slots.where((e) => e != null).length}/4 · 순서대로 표시 (첫 항목 강조)', style: TextStyle(fontSize: Typo.small, color: sub)),
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start,
          ...
          Row(crossAxisAlignment: CrossAxisAlignment.end, children: [
            Expanded(child: Row(crossAxisAlignment: CrossAxisAlignment.center, children: [
              Flexible(child: Text(it.$1, style: TextStyle(fontSize: Typo.body, fontWeight: Typo.medium, color: labelCol))),
              const SizedBox(width: 3),
              GestureDetector(
                behavior: HitTestBehavior.opaque,
              ...
              child: _showCoreHint
                  ? _MarchingAntsBox(
                      ...
                      child: Padding(
                        ...
                        child: Text(it.$2, style: TextStyle(fontSize: Typo.h1, fontWeight: Typo.extra,
                            color: highlight ? tc : ink, fontFeatures: const [FontFeature.tabularFigures()])),
                      ),
                    )
                  : Text(it.$2, style: TextStyle(fontSize: Typo.h1, fontWeight: Typo.extra,
                      color: highlight ? tc : ink, fontFeatures: const [FontFeature.tabularFigures()])),
            ),
          ]),
          if (c != null) ...[
            ...
            Row(children: [
              Text('리그 ${c['rank']}위',
                  style: TextStyle(fontSize: Typo.body, fontWeight: Typo.bold, color: tc)),
              if (c['dom_rank'] != null)
                Text('  ·  국내 ${c['dom_rank']}위',
                    style: TextStyle(fontSize: Typo.small, color: sub)),
            ]),
            const SizedBox(height: 6),
            ClipRRect(
              borderRadius: BorderRadius.circular(Radii.pill),
            ...
            Text('리그 평균 ${_fmtStat(it.$3, (c['lg'] as num?) ?? 0)}',
                style: TextStyle(fontSize: Typo.small, fontWeight: Typo.medium, color: sub)),
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          Expanded(child: Text(
              ...
              style: TextStyle(fontSize: Typo.mini, fontWeight: Typo.bold, color: sub, letterSpacing: 0.5))),
          GestureDetector(
            onTap: () => _openCorePicker(isPitcher),
            child: Row(mainAxisSize: MainAxisSize.min, children: [
              Icon(Icons.tune, size: 13, color: sub),
              const SizedBox(width: 3),
              Text('편집', style: TextStyle(fontSize: Typo.caption, color: sub, fontWeight: Typo.medium)),
            ]),
          ),
        ]),
        if (_showCoreHint) ...[
          Row(children: [
            ...
            Text('점선 숫자를 눌러 리그 평균·설명 보기',
                style: TextStyle(fontSize: Typo.mini, fontWeight: Typo.bold, color: tc)),
          ]),
          const SizedBox(height: 6),
        ],
        const SizedBox(height: Space.xs),
  ...
  Widget _buildPitchDesignCard(Map<String, dynamic> player) {
    ...
    Widget chip(String label, bool active, VoidCallback onTap) => GestureDetector(
      ...
      child: Container(
        ...
        child: Text(label,
            style: TextStyle(fontSize: Typo.caption, fontWeight: active ? Typo.bold : Typo.semibold,
                              ...
                              Expanded(child: Builder(builder: (_) {
                                ...
                                return Container(
                                  ...
                                  child: label.isEmpty
                                      ? null
                                      : Text(label,
                                          style: TextStyle(fontSize: 9.5, fontWeight: Typo.extra,
                                              color: ink,
                                              fontFeatures: const [FontFeature.tabularFigures()])),
                                );
                              })),
                              ...
                              Expanded(child: Builder(builder: (_) {
                                ...
                                return Container(
                                  ...
                                  child: label.isEmpty ? null : Text(label,
                                      style: TextStyle(fontSize: 9.5, fontWeight: Typo.extra, color: ink,
                                          fontFeatures: const [FontFeature.tabularFigures()])),
                                );
                              })),
                          ])),
                      ...
                      leftTitles: AxisTitles(
                        sideTitles: SideTitles(
                          ...
                          getTitlesWidget: (value, _) {
                            ...
                            return Text(text, style: TextStyle(fontSize: Typo.caption, color: Colors.grey[600]));
                          },
                        ),
                      ),
                      bottomTitles: AxisTitles(
                        sideTitles: SideTitles(
                          ...
                          getTitlesWidget: (value, _) {
                            ...
                            return Padding(
                              padding: const EdgeInsets.only(top: 4),
                              child: Text(labels[idx], style: TextStyle(fontSize: Typo.caption, color: Colors.grey[600])),
                            );
                          },
                        ),
                      ),
                        ...
                        getTooltipItems: (touchedSpots) {
                          return touchedSpots.map((spot) {
                            ...
                            return LineTooltipItem(
                              ...
                              const TextStyle(color: Colors.white, fontSize: Typo.caption),
                            );
                          }).toList();
                        },
                      ),
  ...
  Widget _buildRecent5Games(Map<String, dynamic> player) {
    ...
    Widget summaryCell(String label, String value, String key) => Column(children: [
      Text(value,
          style: TextStyle(fontSize: Typo.lg, fontWeight: Typo.extra, color: ink,
      ...
      GestureDetector(
        ...
        child: Row(mainAxisSize: MainAxisSize.min, children: [
          Text(label, style: TextStyle(fontSize: Typo.micro, color: sub)),
          ...
          child: Column(
            ...
            children: [
              Row(children: [
                _sectionLabel('구종 분포'),
                const Spacer(),
                Text('총 $total구', style: TextStyle(fontSize: Typo.caption, color: Colors.grey[600])),
              ]),
              const SizedBox(height: 10),
              ...types.map((t) {
                ...
                return Padding(
                  ...
                  child: Row(children: [
                    SizedBox(
                      width: 56,
                      child: Text(type, style: const TextStyle(fontSize: Typo.small, fontWeight: Typo.medium)),
                    ),
                    Expanded(
                      child: Stack(children: [
                    ...
                    SizedBox(
                      ...
                      child: Text('${pct.toStringAsFixed(1)}% ($cnt)',
                          style: const TextStyle(fontSize: Typo.caption), textAlign: TextAlign.right),
                    ),
                  ]),
                );
              }),
