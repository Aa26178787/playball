...
class _PitchLocationSheetState extends State<PitchLocationSheet> {
  ...
  Future<void> _load() async {
    try {
      ...
      if (mounted) {
        final pitches = (data['pitches'] as List? ?? []).map((p) => Map<String, dynamic>.from(p as Map)).toList();
      ...
      child: Column(
        children: [
          ...
          const Padding(
            ...
            child: Text('투구 위치', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
          ),
          if (_loading)
            ...
            if (_pitchers.length > 1)
              Padding(
                ...
                child: SingleChildScrollView(
                  ...
                  child: Row(
                    children: _pitchers.map((p) {
                      final sel = _selectedPitcher == p;
                      return Padding(
                        padding: const EdgeInsets.only(right: 6),
                        child: ChoiceChip(
                          label: Text(p, style: TextStyle(fontSize: 12, color: sel ? Colors.white : Colors.black87)),
                          selected: sel,
                          selectedColor: const Color(0xFF1A237E),
                          onSelected: (_) => setState(() => _selectedPitcher = p),
                        ),
                      );
                    }).toList(),
                  ),
              ...
              child: SingleChildScrollView(
                ...
                child: Row(
                  children: _resultLabels.entries.map((e) {
                    ...
                    final color = e.key == 'all' ? Colors.grey : (_resultColors[e.key] ?? Colors.grey);
                    return Padding(
                      padding: const EdgeInsets.only(right: 6),
                      child: FilterChip(
                        label: Text(e.value, style: TextStyle(fontSize: 11, color: sel ? Colors.white : Colors.black87)),
                        selected: sel,
                        selectedColor: color,
            ...
            Padding(
              ...
              child: Text('${_filtered.length}구', style: const TextStyle(fontSize: 12, color: Colors.grey)),
            ),
            // 차트
            Expanded(
              child: Padding(
...
class _StrikeZonePainter extends CustomPainter {
  ...
  static const double plateHalfW = 17.0 / 12.0; // 17 inches in feet

  // Use average strike zone from pitches
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawLine(
      ...
      Paint()..color = Colors.brown.withOpacity(0.3)..strokeWidth = 1,
    );
    ...
    canvas.drawRect(zoneRect,
        Paint()..color = Colors.red.withOpacity(0.06));
    canvas.drawRect(zoneRect,
        Paint()..color = Colors.red.withOpacity(0.5)..style = PaintingStyle.stroke..strokeWidth = 1.5);
    ...
    for (int i = 1; i <= 2; i++) {
      ...
      canvas.drawLine(Offset(tl.dx, y), Offset(br.dx, y),
          Paint()..color = Colors.red.withOpacity(0.2)..strokeWidth = 0.5);
    }
    ...
    canvas.drawLine(
      ...
      Paint()..color = Colors.grey.withOpacity(0.2)..strokeWidth = 0.5,
    );
  ...
  @override
  bool shouldRepaint(_StrikeZonePainter old) =>
      old.pitches != pitches;
}
