...
- **06-23~25 우천중단(rain delay) 완성 + DB 커넥션 누수 사고**: 06-23 착수분(BE 부품만, scheduler 미연결+API/앱 미작업)을 06-25 완성·배포(commit `841f417`, 웹+서버 라이브, ⚠️네이티브 APK 미반영). 스펙=`docs/superpowers/specs/2026-06-23-rain-delay-design.md`.
  ...
  - ⚠️⚠️ **DB 커넥션 누수 사고 (배포 중 발견)**: 마이그레이션 `ALTER TABLE games`가 lock_timeout으로 실패 → 조사: **`idle in transaction` 세션 5개가 1~4일째 방치**(pid 521217이 `SELECT ... FROM games WHERE id=494` 트랜잭션 미커밋으로 `games` AccessShare 락 보유 → ALTER의 AccessExclusive 차단, 나머지 4개는 `SELECT DISTINCT pds.player_id...` aborted 상태로 매~24h 1개씩 누적). `pg_terminate_backend`로 5개 정리 후 ALTER 성공. **근본원인=커넥션을 BEGIN 후 commit/rollback 없이 반납하는 코드 경로 존재**(특히 그 `pds`(player_daily_stats) DISTINCT 쿼리 — 일배치 추정). 정리는 일회성, **재발 가능** → ↓알려진 이슈. 교훈: 핫테이블 ALTER 전 `pg_stat_activity` idle-in-tx 점검.
...
## 알려진 이슈
- ⚠️ **DB 커넥션 누수 (idle in transaction 미커밋)** — 06-25 발견: 어떤 경로가 BEGIN 후 commit/rollback 없이 커넥션 반납 → `games`/`player_daily_stats` 등에 락 잔존, 누적 시 핫테이블 ALTER·VACUUM 차단(마이그레이션 lock_timeout 사고). 특히 `SELECT DISTINCT pds.player_id, p.name, t.name, p.player_type...`(player_daily_stats, 일배치 추정)가 매~24h aborted-idle 1개씩 누적. 임시조치=`pg_terminate_backend`로 정리. **미해결 근본원인** — 점검: `SELECT pid,state,now()-state_change,left(query,60) FROM pg_stat_activity WHERE state LIKE 'idle in transaction%'`. 수정 후보=해당 쿼리 경로 `with`/`finally`+commit 보장, 또는 `idle_in_transaction_session_timeout` 설정
