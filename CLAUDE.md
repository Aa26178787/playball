...
### 경기 상세
- [ ] 경기 상세 헤더에 팀명 위에 팀로고 배치
- [ ] 경기 상세에서 미니 게임카드로 다른 경기 상세로 바로 이동
- [ ] 경기 상세 득점 요약 접기/펼치기 탭 + 득점 내용 상세 (예: ㅇㅇㅇ의 우중간 2루타 3타점)
- [ ] 실시간 중계 필드뷰 표시 (네이버처럼, 경기 종료 후에도 유지)
- [ ] 투구위치 보기를 이닝별 중계 각 타자마다도 볼 수 있게 추가

### 선수
- [ ] 선수 상세 시즌 트렌드에서 다른 스탯으로 변경 가능하게
- [ ] 부문별 순위 업데이트 안 되는 버그 확인/수정

### 팀 상세
- [ ] 선수탭 3열 카드 형태 (얼굴+등번호+이름 증명사진 스타일)
- [ ] 재계약 안 한 외인 선수 팀에서 삭제 (과거 경기 기록은 유지)
- [ ] 등말소 탭 중복 표시 제거 (같은 날짜 동일 내용 중복, 웨이버→말소 중복 등)
- [ ] 타순별 탭: 선수 이미지 추가 + 클릭 시 선수 상세 이동

### 캘린더
- [ ] 캘린더 탭 마이팀 일정만 표시
- [ ] 직관 기록 UI 개선: 캘린더 일정 추가 FAB → "일정 추가" / "직관 기록" 분리, 직관 기록에 이미지 첨부

### 순위
- [ ] 순위 스크린에 팀 기록 탭 추가 (팀타율/방어율/WHIP 등 부문별 전체 카테고리)
