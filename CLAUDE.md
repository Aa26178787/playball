...
- **전역 글자크기 = OS 미반영 고정** (06-25): main.dart `MediaQuery.withNoTextScaling`(구 `withClampedTextScaling 0.85~1.1`) → OS '텍스트 크기' 설정이 앱에 영향 없음(고정 1.0). 앱 자체 zoom 1.05(레이아웃+폰트 비례)만 적용. ⚠️ a11y 큰글자 의도 포기(사용자 결정). **picker clamp assert(`'maxScale > minScale'`) 무효화됨**(min/max 없는 noScaling이라) — 단 `_pickerBuilder` linear scaler 패턴은 잔존(무해, 제거 불요). showDatePicker/showTimePicker는 여전히 builder로 scaler 명시 권장
