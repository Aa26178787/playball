...
- [~] 트랙A코어 크롤러 (2026-06-15 GO): **통산스탯 ✅**(`historical_crawler.py` Basic1+Basic2, **1982~2025 배치 실행중** nohup `/tmp/hist_crawl.log`) · **신상bio ✅**(`enrich_bio` detail div.player_info, 손민한 검증) · **franchise링크 ✅**(`link_franchises` 919/919, 백인천→LG 검증). **⚠️배치 완료 후 runbook**: ① `python3 -m crawler.historical_crawler link`(재실행=새 시즌 franchise/debut/final 갱신) ② `python3 -m crawler.historical_crawler bio`(전 선수 bio). 잔여: **세이버 recompute**(war/woba/wrc+/fip — KBO 미제공, 구시즌 입력결손 주의) · **수상/PS**(⚠️aggregation 페이지 서버 IP 차단 → 로컬PC 크롤 필요, insta 패턴) · 비규정선수 완전성
