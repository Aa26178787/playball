...
def _batch_recent5(cur, team_ids: list) -> dict:
    """team_id ??['W','L','D','C',...] ÏµúÏã†??5Í∞?(Ï∑®ÏÜå ?¨Ìï®)"""
    if not team_ids:
        return {}
    cur.execute("""
        SELECT team_id, result FROM (
            ...
            WHERE g.status IN ('Ï¢ÖÎ£å', 'Ï∑®ÏÜå')
        ) sub
        WHERE rn <= 5
        ORDER BY team_id, rn
    """, (team_ids,))
...
# ?¥ÎãùÎ≥?raw Naver JSON Ï∫êÏãú ???ÑÎ£å ?¥Îãù ?ÅÍµ¨ Î≥¥Ï°¥, ?ÑÏû¨ ?¥ÎãùÎß?Îß?Ï£ºÍ∏∞ fetch
_inning_raw_cache: dict = {}  # {naver_game_id: {inning: raw_json}}


@router.get("/{game_id}/relay_all")
def get_game_relay_all(game_id: int):
    # ?Ä?Ä?Ä relay_all ?úÎ≤Ñ?¨Ïù¥??Ï∫êÏãú ?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä
    # Î™©Ï†Å: 1000Î™??ôÏãú?ëÏÜç ??Îß??îÏ≤≠ÎßàÎã§ Naver API ?ÑÏ≤¥ ?¥Îãù ?¨Ï°∞??Î∞©Ï?
    #       ??Naver IP Ï∞®Îã® + ?úÎ≤Ñ Í≥ºÎ???Î∞©Ï?
    # TTL: ÏßÑÌñâÏ§?30Ï¥??¥Îùº?¥Ïñ∏???àÎ°úÍ≥†Ïπ® Ï£ºÍ∏∞?Ä ?ºÏπò), Ï¢ÖÎ£å=3600Ï¥?Î∂àÎ?)
    # ??†ú Í∏àÏ?: Í≥†Î?????Naver Ï∞®Îã® Ï¶âÏãú ?¨Î∞ú
    ...
    conn = get_connection()
    if not conn:
        raise HTTPException(status_code=500, detail="DB ?∞Í≤∞ ?§Ìå®")
    cur = conn.cursor()
    ...
    row = cur.fetchone()
    if not row:
        raise HTTPException(status_code=404, detail="Í≤ΩÍ∏∞Î•?Ï∞æÏùÑ ???ÜÏäµ?àÎã§")
    ...
    if status == 'ÏßÑÌñâ':
        ...
        # ?¥ÎãùÎ≥?raw JSON Ï∫êÏãú: ?ÑÎ£å ?¥Îãù ?¨ÏÇ¨?? ?ÑÏû¨ ?¥ÎãùÎß?Naver fetch
        ...
        # ?ÑÎ£å ?¥Îãù Ï§?Ï∫êÏãú ÎØ∏Î≥¥??+ ?ÑÏû¨ ?¥ÎãùÎß?fetch
        innings_to_fetch = [
        ...
        try:
                ...
                for item in reversed(text_relays):
                    ...
                    for opt in item.get('textOptions', []):
                        ...
                        if batter_record.get('name'):
                            ...
                            m = re.match(r'^(?:\d+Î≤àÌ????Ä?Ä)\s+(\S+)', opt_text)
                            if m:
            ...
            cache_set(_cache_key, result, 30)  # ÏßÑÌñâÏ§? 30Ï¥?Ï∫êÏãú
            return result

        except Exception as e:
            raise HTTPException(status_code=500, detail=f"Ï§ëÍ≥Ñ Ï°∞Ìöå ?§Ìå®: {str(e)}")
    ...
    pitch_title_pattern = re.compile(r'^(\d+)Íµ?s+(.+?)\s+(\d+)km/h\s+(.+)$')

    def parse_pitch_result(title):
        if not title:
            return None
        if '?õÏä§?? in title: return 'S'
        if '?§Ìä∏?ºÏù¥?? in title: return 'S'
        if 'Î≥? in title and 'km/h' in title: return 'B'
        if '?åÏö∏' in title: return 'F'
        if '?ÄÍ≤? in title: return 'X'
        return None
    ...
    for p in pitches:
        ...
        if rtype == 8 and title and 'Î≤àÌ??? in str(title):
            parts = title.split(' ')
            if len(parts) >= 2:
        ...
        if rtype == 2 and title and '?¨Ïàò' in title and 'ÍµêÏ≤¥' in title:
            if inning_half == '0':
    ...
    cache_set(_cache_key, result, 3600)  # Ï¢ÖÎ£å Í≤ΩÍ∏∞: 1?úÍ∞Ñ Ï∫êÏãú (?∞Ïù¥??Î∂àÎ?)
    return result
...
def get_game_preview(game_id: int):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"?ÑÎ¶¨Î∑?Ï°∞Ìöå ?§Ìå®: {str(e)}")
...
def get_game_record_detail(game_id: int):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"?ÅÏÑ∏ Í∏∞Î°ù Ï°∞Ìöå ?§Ìå®: {str(e)}")
...
def get_today_games():
    ...
    # r ?∏Îç±?? 0-15 Í∏∞Ï°¥, 16=win_pitcher, 17=win_pitcher_image, 18=lose_pitcher, 19=lose_pitcher_image, 20=home_starter, 21=away_starter
    recent5_map = _batch_recent5(cur, all_team_ids)

    cur.close()
    conn.close()

    # Íµ¨Ïû•Î≥??†Ïî® Ï∫êÏãú (Í∞ôÏ? Íµ¨Ïû• Ï§ëÎ≥µ ?∏Ï∂ú Î∞©Ï?)
    weather_cache: dict = {}
    ...
    for r in rows:
        ...
        if stadium_id and stadium_id not in weather_cache:
            ...
            if start_time:
                try:
                    ...
                    start_hour = (total_sec // 3600 + 9) % 24  # UTC?íKST
                except Exception:
                    pass
        ...
        games.append({
            ...
            "home_starter":     r[20],
            "away_starter":     r[21],
            "is_draw":          r[2] == 'Ï¢ÖÎ£å' and r[3] == r[4],
        })
...
def get_game_detail(game_id: int):
    ...
    # ?¨Ïàò - ÏßÑÌñâ Ï§ëÏù¥Î©?game_rosters ?†Î∞ú?¨Ïàò, Ï¢ÖÎ£åÎ©?game_pitchers
    if status == 'ÏßÑÌñâ':
        cur.execute("""
    ...
    if status == 'ÏßÑÌñâ' and naver_game_id:
        try:
            inning = game[5] or 1
            url = f"https://api-gw.sports.naver.com/schedule/games/{naver_game_id}/relay?inning={inning}"
...
def get_games_by_date(date_str: str):
    ...
    # stadium_id + team_id Î≥ÑÎèÑ Ï°∞Ìöå
    ...
    # ?§Îäò ?†Ïßú Í≤ΩÍ∏∞Îß??†Ïî® ?¨Ìï® (Í≥ºÍ±∞ ?†Ïßú???§ÌÇµ)
    ...
    for r in rows:
        ...
        status = r[2]
        db_weather = r[15]   # games.weather DB Ïª¨Îüº (Ï¢ÖÎ£å ???Ä??
        has_lineup = r[16]
        win_pitcher = r[13]
        lose_pitcher = r[14]

        if status == '?àÏ†ï' and has_lineup:
            status = '?ºÏù∏??

        # ?†Ïî®: Í≥ºÍ±∞=DB, ?§Îäò=?§ÏãúÍ∞? ÎØ∏Îûò=?àÎ≥¥
        weather = None
    ...
    # ?†ÏßúÎ≥??úÎ≤ÑÏ∫êÏãú TTL: Í≥ºÍ±∞=86400s, ?§Îäò=30s, ÎØ∏Îûò=3600s
    if date_str < _today_str:
        _ttl = 86400
    elif is_today:
        _ttl = 30
...
def get_game_relay(game_id: int):
    ...
    if status != 'ÏßÑÌñâ':
        raise HTTPException(status_code=400, detail="ÏßÑÌñâ Ï§ëÏù∏ Í≤ΩÍ∏∞Í∞Ä ?ÑÎãô?àÎã§")

    inning = current_inning or 1

    _POS_CODE = {
        '?¨Ïàò': 'P', '?¨Ïàò': 'C', '1Î£®Ïàò': '1B', '2Î£®Ïàò': '2B',
        '?†Í≤©??: 'SS', '3Î£®Ïàò': '3B', 'Ï¢åÏùµ??: 'LF', 'Ï§ëÍ≤¨??: 'CF',
        '?∞Ïùµ??: 'RF', 'ÏßÄÎ™ÖÌ???: 'DH',
    }
    ...
    try:
        ...
        if not data.get("success"):
            raise HTTPException(status_code=500, detail="?§Ïù¥Î≤?API ?ëÎãµ ?§Ìå®")

        relay = data["result"]["textRelayData"]
        game_state = relay.get("currentGameState", {}) or {}

        # ?Ä?Ä field_view: ?Ä??Ï£ºÏûê/?òÎπÑ9Î™??Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä
        home_or_away = relay.get("homeOrAway", "0")
        # "0" = away batting (Ï¥?, "1" = home batting (Îß?
        ...
        try:
            ...
            if fconn:
                ...
                # 1) naver_player_id ??player info (?Ä???¨Ïàò???§Ï†ú naver_player_id)
                #    base1/2/3: 1~9?¥Î©¥ ?Ä??Î≤àÌò∏(batting_order), ?ÑÎãàÎ©?naver_player_id
                ...
                # ?Ä??Î≤àÌò∏ Î∞©Ïãù(1-9)??Ï£ºÏûê??game_batters batting_orderÎ°?Ï°∞Ìöå
                base_nids = []
                base_orders = []
                for k in base_keys:
                ...
                # ?Ä??Î≤àÌò∏ ??DB game_batters Ï°∞Ìöå (batting_side = Í≥µÍ≤©?Ä)
        ...
        raise HTTPException(status_code=500, detail=f"Ï§ëÍ≥Ñ ?∞Ïù¥??Ï°∞Ìöå ?§Ìå®: {str(e)}")
...
def get_pitch_locations(game_id: int):
    ...
    if not row or not row[0]:
        ...
        raise HTTPException(status_code=404, detail="Í≤ΩÍ∏∞ ?ÜÏùå")
    naver_game_id = row[0]
    game_status = row[1]
    ...
    away_team = team_row[1] if team_row else "?êÏ†ï"
    ...
    # Ï¢ÖÎ£å Í≤ΩÍ∏∞ ??DB ?∞ÏÑ† Ï°∞Ìöå
    if game_status == 'Ï¢ÖÎ£å':
        cur.execute("""
        ...
        if db_rows:
            cur.close(); conn.close()
            import re as _re
            def _strip_order(name): return _re.sub(r'^\d+Î≤àÌ???', '', name or '')
            all_pitches = [
                {
    ...
    def classify(text):
        ...
        if "Î≥? in text: return "ball"
        if "?õÏä§?? in text or "?§Ïúô" in text: return "swing"
        if "?åÏö∏" in text: return "foul"
        if "?ÄÍ≤? in text or "?àÌ?" in text or "?àÎü∞" in text or "2Î£®Ì?" in text or "3Î£®Ì?" in text: return "hit"
        if "?§Ìä∏?ºÏù¥?? in text: return "strike"
        return "other"
    ...
    def _fetch_and_parse_inning(inning):
        ...
        for item in reversed(text_relays):
            ...
            import re as _re2
            batter = _re2.sub(r'^\d+Î≤àÌ???', '', batter_raw)
    ...
    # Ï¢ÖÎ£å Í≤ΩÍ∏∞?∏Îç∞ DB ?∞Ïù¥???ÜÏóà?ºÎ©¥ ??Î∞©Í∏à fetch???∞Ïù¥???Ä??    if game_status == 'Ï¢ÖÎ£å' and all_pitches:
        try:
...
def get_game_weather(game_id: int):
    ...
    if start_time:
        try:
            ...
            pass
    if status in ('?àÏ†ï', '?ºÏù∏??) and start_hour is not None:
...
def get_game_highlights(game_id: int):
    ...
    if not rows:
        # DB???ÜÏúºÎ©??§ÏãúÍ∞?Google News RSS Ï°∞Ìöå
        cur.execute("""
        ...
        try:
            ...
            import re
            articles = fetch_highlight_rss(f'{home_name} {away_name} ?òÏù¥?ºÏù¥??)
            for a in articles:
...
def predict_game(game_id: int, body: PredictionBody,
    ...
    if status == 'Ï¢ÖÎ£å':
        cur.close(); conn.close()
        raise HTTPException(status_code=400, detail='Ï¢ÖÎ£å??Í≤ΩÍ∏∞???àÏ∏° Î∂àÍ?')
    if body.predicted_team_id not in (home_id, away_id):
        cur.close(); conn.close()
        raise HTTPException(status_code=400, detail='?¥Îãπ Í≤ΩÍ∏∞ ?Ä ?ÑÎãò')
